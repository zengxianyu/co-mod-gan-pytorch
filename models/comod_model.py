...
class CoModModel(torch.nn.Module):
    ...
    def modify_commandline_options(parser, is_train):
        ...
        parser.add_argument('--path_objectshape_base', type=str, required=False, help='path obj base')
        parser.add_argument('--path_objectshape_list', type=str, required=False, help='path obj list')
    ...
    def forward(self, data, mode):
        ...
        if mode == 'generator':
            ...
            return g_loss, real_image, generated
        elif mode == 'dreal':
            d_loss = self.compute_discriminator_loss(
                real_image, fake_image=None, mask=mask)
            return d_loss
        elif mode == 'dfake':
            with torch.no_grad():
            ...
            d_regs = self.compute_discriminator_reg(real_image, mask)
            return d_regs
        elif mode == 'g_reg':
            ...
            with torch.no_grad():
                if self.opt.factor is None:
                    fake_image, uc_image,_ = self.generate_fake(real_image, mask, ema=True)
                else:
                    fake_image, _ = self.factorize_fake(real_image, mask)
            inp = real_image*(1-mask)
            return fake_image, inp
        else:
    ...
    def compute_generator_reg(self, real_image, mask, mean_path_length):
        G_regs = {}
        bsize = real_image.size(0)
        path_batch_size = max(1, bsize // self.opt.path_batch_shrink)
        fake_image, _, latents = self.generate_fake(real_image, mask, True)
    ...
    def compute_discriminator_reg(self, real_image, mask):
        real_image.requires_grad = True
        real_pred = self.netD(real_image, mask)
    ...
    def compute_discriminator_loss(self, real_image, fake_image=None, mask=None):
        D_losses = {}
        ...
        if fake_image is not None:
            ...
            pred_real = self.netD(real_image, mask)
            D_losses['D_real'] = self.criterionGAN(pred_real, True,
                                                   for_discriminator=True)

        return D_losses

    def factorize_fake(self, real_image, mask, return_latents=False):
        self.netG_ema.eval()
        bsize = real_image.size(0)
        latent_in = torch.randn(bsize, self.opt.z_dim, device=self.device)
        dlatent = self.netG_ema(latents_in=[latent_in], get_latent=True)[0]

        direction = self.opt.factor_d * self.eigvec[:, self.opt.factor_i].unsqueeze(0)
        img1, _, latent = self.netG_ema(
                real_image,
                mask,
                [dlatent-direction],
