...
class G_synthesis_co_mod_gan(nn.Module):
    def __init__(
        ...
        class AttentionPool2d(nn.Module):
            ...
            def forward(self, x):
                x = F.interpolate(x, (self.spacial_dim, self.spacial_dim))
        ...
        class E_block_final_conv(nn.Module): # res = 2..resolution_log2
            def __init__(self):
                ...
                self.dropout = nn.Dropout(opt.dropout_rate)
                if opt.att_pool:
                    self.att_pool = AttentionPool2d(spacial_dim=8,
                            embed_dim=nf(1)*2, num_heads=32, output_dim=nf(1)*2)
            def forward(self, data):
                x, E_features = data
                x = self.Conv(x)
                E_features[2] = x
...
class Generator(BaseNetwork):
    ...
    def modify_commandline_options(parser, is_train):
        ...
        parser.add_argument('--att_pool', action='store_true')
        return parser
    def __init__(
            self,
